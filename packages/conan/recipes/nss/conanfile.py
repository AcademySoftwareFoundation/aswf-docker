...
class NSSConan(ConanFile):
    ...
    def build_requirements(self):
        if self.settings.compiler == "Visual Studio" and not tools.get_env(
            "CONAN_BASH_PATH"
        ):
        ...
        if hasattr(self, "settings_build"):
            self.build_requires(
                f"sqlite3/{os.environ['ASWF_SQLITE3_VERSION']}@{self.user}/{self.channel}"
            )
    ...
    def requirements(self):
        self.requires(
            f"nspr/{os.environ['ASWF_NSPR_VERSION']}@{self.user}/{self.channel}"
        )
        self.requires(
            f"sqlite3/{os.environ['ASWF_SQLITE3_VERSION']}@{self.user}/{self.channel}"
        )
        self.requires(
            f"zlib/{os.environ['ASWF_ZLIB_VERSION']}@{self.user}/{self.channel}"
        )
    ...
    def _make_args(self):
        ...
        args.append(
            "OS_TARGET=%s" % os_map.get(str(self.settings.os), "UNSUPPORTED_OS")
        )
        ...
        def _format_libraries(libraries, settings):
            ...
            for library in libraries:
                if (
                    str(compiler) == "Visual Studio"
                    or str(compiler_base) == "Visual Studio"
                ):
        ...
        args.append(
            ...
            % " ".join(
                ...
                + _format_library_paths(
                    self.deps_cpp_info["zlib"].lib_paths, self.settings
                )
            )
        )
        ...
        args.append(
            "SQLITE_INCLUDE_DIR=%s" % self.deps_cpp_info["sqlite3"].include_paths[0]
        )
    ...
    def package(self):
        self.copy(
            "COPYING", src=os.path.join(self._source_subfolder, "nss"), dst="licenses"
        )
        with chdir(self, os.path.join(self._source_subfolder, "nss")):
        ...
        for dll_file in glob.glob(os.path.join(self.package_folder, "lib", "*.dll")):
            rename(
                self,
                dll_file,
                os.path.join(self.package_folder, "bin", os.path.basename(dll_file)),
            )
    ...
    def package_info(self):
        ...
        self.cpp_info.components["softokn"].requires = [
            "sqlite3::sqlite3",
            "nssutil",
            "nspr::nspr",
        ]
