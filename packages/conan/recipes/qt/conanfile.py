...
class QtConan(ConanFile):
    ...
    def configure(self):
        ...
        # Define the submodules we need for ASWF build and PySide in particular
    ...
    def validate(self):
        ...
        if (
            ...
            raise ConanInvalidConfiguration(
                "qt 6.5.0 cannot be built with gcc 9, cf QTBUG-112920"
            )
        ...
        if self.settings.compiler == "clang" and "libstdc++" in str(
        ...
        if (
            ...
            and self.dependencies["double-conversion"].options.shared
        ):
            raise ConanInvalidConfiguration(
                "Test recipe fails because of Macos' SIP. Contributions are welcome."
            )
    ...
    def generate(self):
        ...
        if self.settings.os == "Windows":
            tc.variables["HOST_PERL"] = self.dependencies.build[
                "strawberryperl"
            ].conf_info.get("user.strawberryperl:perl", check_type=str)
    ...
    def source(self):
        ...
        replace_in_file(
            self,
            os.path.join(
                self.source_folder, "qtbase", "cmake", "QtInternalTargets.cmake"
            ),
            "-Zc:wchar_t",
        ...
        if Version(self.version) <= "6.4.0":
            ...
            replace_in_file(
                self,
                os.path.join(
                    self.source_folder, "qtbase", "src", "gui", "configure.cmake"
                ),
                "FONTCONFIG_FOUND",
                "Fontconfig_FOUND",
            )
        ...
        replace_in_file(
            self,
            os.path.join(self.source_folder, "qtbase", "cmake", "QtAutoDetect.cmake"),
    ...
    def package(self):
        ...
        # We want to be able to easily consume Conan packages using CMake outside of Conan itself
        ...
        for m in os.listdir(os.path.join(self.package_folder, "lib", "cmake")):
            ...
            # Same as above, don't limit exported CMake files
        ...
        if self.options.qtdeclarative:
            ...
            targets.extend(
                ["qmlformat", "qml", "qmlprofiler", "qmlpreview", "qmltestrunner"]
            )
    ...
    def package_info(self):
        ...
        self.runenv_info.define(
            "QT_PLUGIN_PATH",
            os.path.join(self.package_folder, "res", "archdatadir", "plugins"),
        )
        ...
        self.cpp_info.components["qtPlatform"].names[
            "cmake_find_package_multi"
        ] = "Platform"
        ...
        if self.options.gui:
            ...
            if self.settings.os == "Windows":
                ...
                _create_plugin(
                    "QCocoaIntegrationPlugin", "qcocoa", "platforms", ["Core", "Gui"]
                )
        ...
        if not self.options.shared:
            if self.settings.os == "Windows":
                ...
                self.cpp_info.components["qtNetwork"].system_libs.extend(
                    ["winhttp", "secur32"]
                )
