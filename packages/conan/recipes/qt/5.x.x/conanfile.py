...
#
# From: https://github.com/conan-io/conan-center-index/blob/8abb942c5fb4acd481548950143024fdcf29ef8f/recipes/qt/5.x.x/conanfile.py
...
class QtConan(ConanFile):
    ...
    default_options = {
        ...
        "with_libpng": True,
        "with_sqlite3": True,
        "with_mysql": False, # ASWF: skip database support
    ...
    def configure(self):
        ...
        self.options.qtwebengine = False # Qt 5 needs Python 2 to buid, OpenRV to use Qt 6
        self.options.qtwebchannel = False
        self.options.qtwebsockets = False
    ...
    def requirements(self):
        ...
        if self.options.get_safe("with_libjpeg", False) and not self.options.multiconfiguration:
            if self.options.with_libjpeg == "libjpeg-turbo":
                self.requires("libjpeg-turbo/[>=3.0 <3.1]", transitive_libs=True) # ASWF: client's don't link o therwise
            else:
        ...
        if self.options.get_safe("with_md4c", False):
            self.requires("md4c/[>=0.4.8 <1]", transitive_libs=True) # ASWF otherwise does not link
...
Prefix = ..""")
        ...
        copy(self, "*LICENSE*", os.path.join(self.source_folder, "qt5/"), os.path.join(self.package_folder, "licenses",self.name),
