...
class TestPackageConan(ConanFile):
    ...
    def test(self):
        if hasattr(self, "settings_build"):
            ...
            m4_bin = os.path.join(
                self.deps_cpp_info["m4"].rootpath, "bin", "m4" + exe_suffix
            )
        else:
            m4_bin = tools.get_env("M4")
            if m4_bin is None or not m4_bin.startswith(
                self.deps_cpp_info["m4"].rootpath
            ):
        ...
        if not tools.cross_building(self, skip_x64_x86=True):
            ...
            test_package_dir = os.path.join(
                self.source_folder, os.pardir, "test_package"
            )
            self.run(
                f"{m4_bin} -R {test_package_dir}/frozen.m4f {test_package_dir}/test.m4",
