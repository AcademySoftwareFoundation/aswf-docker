...
class GlfwConan(ConanFile):
    ...
    def validate(self):
        if (
            self.options.get_safe("with_wayland")
            and not self.dependencies["xkbcommon"].options.with_wayland
        ):
    ...
    def package(self):
        copy(
            ...
            "LICENSE*",
            self.source_folder,
            os.path.join(self.package_folder, "licenses", self.name),
        )
        cmake = CMake(self)
        cmake.install()
    ...
    def _module_file_rel_path(self):
        return os.path.join(
            "lib64", "cmake", f"conan-official-{self.name}-targets.cmake"
        )
